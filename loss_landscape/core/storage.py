...
class LandscapeStorage:
    ...
    def save_hessian(self, hessian_data: Dict[str, Any]):
        ...
        for i, epoch in enumerate(epochs):
            ...
            data.append([epoch, max_eigs[i], traces[i], eigs_json])
    ...
    def export_for_frontend(self, output_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        if len(surface_df) > 0:
            ...
            result["baseline_loss"] = (
                float(baseline_loss_2d) if baseline_loss_2d is not None else float(loss_grid.min())
            )
        ...
        if len(trajectory_df) > 0:
            ...
            if "loss" in trajectory_df.columns:
                ...
                if any(loss_val is not None for loss_val in losses):
