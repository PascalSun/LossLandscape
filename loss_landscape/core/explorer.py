...
class Explorer:
    ...
    使用示例:
        ...
        # 基本用法 - loss_fn 签名: (model, inputs, targets) -> loss
        ...
        with Explorer(model, loss_with_reg, data_loader) as explorer:
            explorer.build_surface(grid_size=50, range_scale=0.1)
        ...
        with Explorer(model, physics_loss, data_loader) as explorer:
            explorer.build_surface(grid_size=50, range_scale=0.1)
        ```
    """
    ...
    def __init__(
        ...
        Args:
            ...
            max_batches: 评估时使用的最大batch数，默认10。设为None则使用全部数据。
        """
        self.model = model
    ...
    def build_hessian_trajectory(
        ...
        hessian_calc = HessianCalculator(
            self.model, self.loss_fn, self.data_loader, device=self.device, max_batches=max_batches
        )

        results = {"epochs": [], "max_eigenvalue": [], "trace": [], "top_eigenvalues": []}
