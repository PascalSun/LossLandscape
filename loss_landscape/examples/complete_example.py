...
def generate_landscape(
    ...
    if metadata_info:
        full_metadata = {
            **metadata_info,
            "loss_function": {"name": loss_name, **loss_description},
        ...
        if training_stats:
            full_metadata["training_statistics"] = {
                ...
                "loss_reduction": (
                    ...
                    if training_stats["initial_loss"] and training_stats["final_loss"]
                    else None
                ),
                "loss_reduction_percent": (
                    float(
                        (training_stats["initial_loss"] - training_stats["final_loss"])
                        / training_stats["initial_loss"]
                        * 100
                    )
                    if training_stats["initial_loss"]
                    and training_stats["final_loss"]
                    and training_stats["initial_loss"] > 0
                    else None
                ),
    ...
    with Explorer(
        model,
        custom_loss_fn,  # loss_fn signature: (model, inputs, targets) -> loss
        data_loader,
        device=device,
        storage=storage,
        model_mode="eval",
...
def main():
    ...
    def mse_loss_fn(model, inputs, targets):
    ...
    def train_and_record(loss_key: str, train_epochs: int = 100):
        ...
        def mse_step_loss_fn(m, x, y):
            return nn.functional.mse_loss(m(x), y)

        def physics_step_loss_fn(m, x, y):
            return physics_loss_fn(m, x, y, nu=nu_val)

        if loss_key == "mse":
            step_loss_fn = mse_step_loss_fn
            lr = 1e-3  # MSE可以稍微大一点
        elif loss_key == "physics":
            step_loss_fn = physics_step_loss_fn
    ...
    base_metadata = {
        ...
        "dataset": {
            ...
            "samples": {"train": len(train_dataset), "eval": len(eval_dataset)},
        },
        "model": {
        ...
        "training": {
            ...
            "scheduler": {"name": "StepLR", "step_size": 20, "gamma": 0.5},
            "regularization": {
