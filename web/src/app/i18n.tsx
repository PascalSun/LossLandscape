/* Minimal client-side i18n (EN/中文) with a header language toggle. */
'use client';

import React, { createContext, useContext, useEffect, useMemo, useState } from 'react';

export type Locale = 'en' | 'zh';

const DICT = {
  en: {
    appName: 'LearnableKGE',
    tagline: 'Loss Landscape',
    navGenerate: 'Generate',
    navViewer: 'Viewer',
    panelGenerateTitle: 'Generate',
    inputSource: 'Input source',
    fromConfig: 'From Config',
    fromRun: 'From Run',
    configPath: 'Config path',
    runDir: 'Run directory',
    repoRelative: 'repo-relative',
    loadFromRunHint: 'Select a run directory to load landscape data from outputs/run_xxx/',
    noRunsFound: 'No runs found',
    loading: 'Loading…',
    optional: 'optional',
    runHint: 'If run is set, config is ignored; config + trajectory are extracted from the run folder.',
    mode: 'Generation Mode',
    mode2D: '2D Grid',
    mode3D: '3D Volume',
    grid: 'Grid',
    direction: 'Direction',
    generate: 'Generate',
    generating: 'Generating…',
    perfHint: 'Tip: 3D + grid=50 can be slow; try 30 first.',
    perfPresets: 'Quick Presets',
    presetFast: '⚡ Fast',
    presetStandard: '📊 Standard',
    presetHQ: '💎 High Quality',
    maxBatches: 'Max Batches',
    viewerTitle: 'Viewer',
    noDataTitle: 'No data yet',
    noDataDesc: 'Select a run directory to load landscape data.',
    loadingOverlay: 'Generating loss landscape…',
    historyTitle: 'History',
    historyHint: 'Load previously generated results (stored in DuckDB).',
    refresh: 'Refresh',
    load: 'Load',
    importTitle: 'Import',
    importHint: 'Import JSON files generated by loss landscape generation. Upload a file or scan a folder to automatically import all JSON files.',
    import: 'Import',
    uploadFile: 'Upload File',
    uploadHint: 'Upload a JSON file from your computer',
    selectFile: 'Select JSON file',
    fileSelected: 'File selected',
    scanFolder: 'Scan Folder',
    scanFolderHint: 'Scan a run directory (e.g., outputs/run_xxx) to automatically import all JSON files',
    folderPath: 'Folder path',
    scanAndImport: 'Scan & Import All',
    scanning: 'Scanning…',
    importing: 'Importing…',
    filesFound: 'files found',
    filesImported: 'files imported',
    viewMode: 'View Mode',
    view2D: '2D Heatmap',
    viewSurface: '2D',
    viewSurfaceSlice: '3D Surface (γ slice)',
    viewSlice: '3D',
    viewVolume: '3D',
    view3DRenderMode: '3D Render Mode',
    view3DSlice: 'Slice',
    view3DVolume: 'Volume Render',
    gamma: 'γ',
    opacity: 'Opacity',
    step: 'Step',
    lowerIsBetter: 'Lower is better',
    view2DDesc: 'Shows loss in 2D (α, β). Good for understanding loss variations in two directions.',
    view3DSurfaceDesc: 'Shows loss as 3D surface (α, β, loss). Good for seeing the overall shape.',
    view3DSurfaceSliceDesc: 'Pick a γ slice of the 3D volume and view it as a 3D surface. Use the slider to move through slices.',
    view3DSliceDesc: 'Shows 2D slices of 3D volume at different γ values. Reveals how loss changes along the third dimension.',
    view3DVolumeDesc: 'Shows all slices blended together. Reveals 3D structure and high-loss regions.',
    themeToggleDark: 'Switch to dark mode',
    themeToggleLight: 'Switch to light mode',
    deleteLandscape: 'Delete Landscape',
    deleteConfirm: 'Are you sure you want to delete this landscape? This action cannot be undone.',
    deleteFailed: 'Delete Failed',
    deleteFailedMessage: 'Failed to delete',
    previouslyGenerated: 'Previously generated landscapes',
    noHistoryFound: 'No history found',
    delete: 'Delete',
    runDirectory: 'Run Directory',
    file: 'File',
    imported: 'Imported',
    created: 'Created',
    uploadLandscapeHint: 'Upload a landscape JSON file. The file will be validated before import.',
    dropFileHere: 'Drop file here',
    dragDropFile: 'Drag & drop file here',
    orClickToBrowse: 'or click to browse',
    supportsJsonOnly: 'Supports .json files only',
    clickToChangeFile: 'Click to change file',
    size: 'Size',
    uploadingAndImporting: 'Uploading and importing...',
    uploadAndImport: 'Upload & Import',
    fileMustBeJson: 'File must be a .json file',
    pleaseSelectFile: 'Please select a file first',
    uploadFailed: 'Upload failed',
    fileUploadedSuccess: 'File uploaded and imported successfully!',
    loading3D: 'Loading 3D visualization...',
    loading2D: 'Loading 2D visualization...',
    loading1D: 'Loading 1D visualization...',
    loadingVolumeSlice: 'Loading volume slice...',
    loadingVolumeRender: 'Loading volume render...',
    loadingHessian: 'Loading Hessian analysis...',
    runMetadata: 'Run Metadata',
    comprehensiveInfo: 'Comprehensive information about this experiment',
    summary: 'Summary',
    overview: 'Overview',
    factorsAffectingLoss: 'Factors Directly Affecting Loss Landscape',
    emptyArray: 'empty array',
    arrayItems: 'Array',
    hessianAnalysis: 'Hessian Analysis',
    hessianStaticDesc: 'Static analysis of the loss landscape curvature at the current checkpoint.',
    hessianEvolutionDesc: 'Evolution of loss landscape curvature throughout training, analyzing the Hessian spectrum.',
    spectralMonitor: 'Spectral Monitor',
    spectralMonitorHint: 'Epoch {epoch} · λmax / Trace · slide to explore spectrum history',
    curvatureDynamics: 'Curvature Dynamics',
    lambdaMaxSharpness: 'λmax (Sharpness): Largest eigenvalue of the Hessian, tracking the sharpest direction in the loss landscape.',
    stabilityBoundary: 'Stability Boundary (2/η): If λmax exceeds this, training may become unstable for plain SGD.',
    traceDesc: 'Trace: Sum of eigenvalues, representing total curvature around the current point.',
    sharpness: 'Sharpness (λmax)',
    stability: 'Stability (2/η)',
    totalCurvature: 'Total Curvature (Trace)',
    spectralAnalysis: 'Spectral Analysis',
    spectralDensityDesc: 'Spectral Density: KDE of Hessian eigenvalues; the current epoch is highlighted.',
    screePlotDesc: 'Scree Plot: Top eigenvalues (algebraic order) to separate bulk spectrum from outliers.',
    spectralDensity: 'Spectral Density (KDE)',
    epoch: 'Epoch',
    history: 'History',
    noDensityData: 'No density data available',
    topKSpectrum: 'Top-k Spectrum (Ranked)',
    rank: 'Rank',
    eigenvalue: 'Eigenvalue',
    noSpectrumData: 'No spectrum data available',
    density: 'Density',
    currentPosition: 'Current Position',
    trainingTrajectory: 'Training Trajectory',
    baselineLoss: 'Baseline Loss',
    highLoss: 'High Loss',
    lowLoss: 'Low Loss',
    scale: 'Scale',
    log: 'Log',
    linear: 'Linear',
    tipHover: 'Tip: Move mouse over the plot to see exact values',
    tip3D: 'Tip: Drag to rotate, scroll to zoom, hover to see values',
    hoverForHint: 'Hover for hint',
    lossLandscape2D: 'Loss Landscape (2D)',
    lossLandscape3D: 'Loss Landscape (3D)',
    lowerIsBetterArrow: '↓ Lower is better ↓',
    noData: 'No Data',
    tabRun: 'Run',
    tabHistory: 'History',
    tabUpload: 'Upload',
    loss: 'Loss',
    trainLoss: 'Train Loss',
    validationLoss: 'Validation Loss',
    alpha: 'α',
    beta: 'β',
    log10Loss: 'log₁₀(loss)',
    sectionExperiment: 'Experiment',
    sectionDataset: 'Dataset',
    sectionModel: 'Model',
    sectionTraining: 'Training',
    sectionTrainingStatistics: 'Training Statistics',
    sectionLossFunction: 'Loss Function',
    sectionLandscapeGeneration: 'Landscape Generation',
    sectionSystemInformation: 'System Information',
    alphaCurve: 'Alpha curve',
    alphaFloor: 'Alpha floor',
    metrics: 'Metrics',
    trainingProgression: 'Training progression',
    noHessianMetrics: 'This run does not have Hessian metrics.',
    noHessianDataAvailable: 'No Hessian Data Available',
    view1D: '1D',
    view1DDesc: '1D Loss Landscape visualization',
    viewHessian: 'Hessian',
    viewHessianSnapshot: 'Hessian snapshot (no trajectory)',
    viewHessianMetrics: 'Hessian metrics over training (λmax, Trace, Spectrum)',
    viewMetadata: 'Metadata',
    viewMetadataDesc: 'View comprehensive run metadata',
    slices: 'slices',
    volumeRenderingDesc: 'Volume rendering: all slices blended together. Brighter = higher loss. Rotate to see 3D structure.',
    landscapeLoss: 'Landscape Loss',
    valueLabelTrain: 'train',
    valueLabelVal: 'val',
    valueLabelLand: 'land',
    no1DLossData: 'This run does not have 1D loss landscape data.',
    items: 'items',
    modalOK: 'OK',
    modalCancel: 'Cancel',
    noMetadataAvailable: 'No Metadata Available',
    noMetadataInfo: 'This run does not have metadata information.',
    no1DDataAvailable: 'No 1D Data Available',
    notAvailable: 'N/A',
    sliceIndex: 'Slice Index',
    alphaSlice: 'α-slice',
    betaSlice: 'β-slice',
    gammaSlice: 'γ-slice',
    alphaIdx: 'α idx',
    betaIdx: 'β idx',
    gammaIdx: 'γ idx',
    axis: 'Axis',
    threshold: 'Threshold',
    dataMinMax: 'Data min/max:',
  },
  zh: {
    appName: 'LearnableKGE',
    tagline: 'Loss Landscape',
    navGenerate: '生成',
    navViewer: '预览',
    panelGenerateTitle: '生成',
    inputSource: '输入来源',
    fromConfig: '从 Config',
    fromRun: '从 Run',
    configPath: 'Config 路径',
    runDir: 'Run 目录',
    repoRelative: '相对仓库路径',
    loadFromRunHint: '选择一个 run 目录，从 outputs/run_xxx/ 加载 landscape 数据',
    noRunsFound: '未找到 run',
    loading: '加载中…',
    optional: '可选',
    runHint: '如果填写 run，会忽略 config，并从 run 目录自动提取 config + trajectory。',
    mode: '生成模式',
    mode2D: '2D 网格',
    mode3D: '3D 体积',
    grid: '网格',
    direction: '方向',
    generate: '生成',
    generating: '生成中…',
    perfHint: '提示：3D + grid=50 会比较慢，建议先用 30。',
    perfPresets: '快速预设',
    presetFast: '⚡ 快速',
    presetStandard: '📊 标准',
    presetHQ: '💎 高质量',
    maxBatches: '最大批次',
    viewerTitle: '预览',
    noDataTitle: '暂无数据',
    noDataDesc: '选择一个 run 目录以加载 landscape 数据。',
    loadingOverlay: '正在生成 loss landscape…',
    historyTitle: '历史记录',
    historyHint: '加载已生成结果（存储在 DuckDB）。',
    refresh: '刷新',
    load: '载入',
    importTitle: '导入',
    importHint: '导入 loss landscape 生成产生的 JSON 文件。可以上传单个文件或扫描文件夹自动导入所有 JSON 文件。',
    import: '导入',
    uploadFile: '上传文件',
    uploadHint: '从您的电脑上传 JSON 文件',
    selectFile: '选择 JSON 文件',
    fileSelected: '已选择文件',
    scanFolder: '扫描文件夹',
    scanFolderHint: '扫描 run 目录（如 outputs/run_xxx）自动导入所有 JSON 文件',
    folderPath: '文件夹路径',
    scanAndImport: '扫描并导入全部',
    scanning: '扫描中…',
    importing: '导入中…',
    filesFound: '个文件',
    filesImported: '个文件已导入',
    viewMode: '视图模式',
    view2D: '2D 热图',
    viewSurface: '2D',
    viewSurfaceSlice: '3D 曲面(γ切片)',
    viewSlice: '3D',
    viewVolume: '3D',
    view3DRenderMode: '3D 渲染方式',
    view3DSlice: '切片',
    view3DVolume: '体渲染',
    gamma: 'γ',
    opacity: '透明度',
    step: '步长',
    lowerIsBetter: '越低越好',
    view2DDesc: '显示2D loss landscape (α, β)。适合理解两个方向上的loss变化。',
    view3DSurfaceDesc: '显示3D loss 曲面 (α, β, loss)，便于整体观察形状。',
    view3DSurfaceSliceDesc: '对3D体积按γ取切片，用3D曲面展示。拖动滑条在不同切片间切换。',
    view3DSliceDesc: '显示3D体积在不同γ值下的2D切片。揭示loss在第三个维度上的变化。',
    view3DVolumeDesc: '显示所有切片混合叠加。揭示3D结构和高loss区域。',
    themeToggleDark: '切换到深色模式',
    themeToggleLight: '切换到浅色模式',
    deleteLandscape: '删除 Landscape',
    deleteConfirm: '您确定要删除这个 landscape 吗？此操作无法撤销。',
    deleteFailed: '删除失败',
    deleteFailedMessage: '删除失败',
    previouslyGenerated: '之前生成的 landscapes',
    noHistoryFound: '未找到历史记录',
    delete: '删除',
    runDirectory: 'Run 目录',
    file: '文件',
    imported: '已导入',
    created: '已创建',
    uploadLandscapeHint: '上传一个 landscape JSON 文件。文件将在导入前进行验证。',
    dropFileHere: '将文件拖放到此处',
    dragDropFile: '拖放文件到此处',
    orClickToBrowse: '或点击浏览',
    supportsJsonOnly: '仅支持 .json 文件',
    clickToChangeFile: '点击更改文件',
    size: '大小',
    uploadingAndImporting: '正在上传并导入...',
    uploadAndImport: '上传并导入',
    fileMustBeJson: '文件必须是 .json 文件',
    pleaseSelectFile: '请先选择文件',
    uploadFailed: '上传失败',
    fileUploadedSuccess: '文件已成功上传并导入！',
    loading3D: '正在加载 3D 可视化...',
    loading2D: '正在加载 2D 可视化...',
    loading1D: '正在加载 1D 可视化...',
    loadingVolumeSlice: '正在加载体积切片...',
    loadingVolumeRender: '正在加载体积渲染...',
    loadingHessian: '正在加载 Hessian 分析...',
    runMetadata: 'Run 元数据',
    comprehensiveInfo: '关于此实验的全面信息',
    summary: '摘要',
    overview: '概览',
    factorsAffectingLoss: '直接影响 Loss Landscape 的因素',
    emptyArray: '空数组',
    arrayItems: '数组',
    hessianAnalysis: 'Hessian 分析',
    hessianStaticDesc: '当前检查点的损失景观曲率静态分析。',
    hessianEvolutionDesc: '训练过程中损失景观曲率的演变，分析 Hessian 谱。',
    spectralMonitor: '谱监控器',
    spectralMonitorHint: 'Epoch {epoch} · λmax / Trace · 滑动以探索谱历史',
    curvatureDynamics: '曲率动力学',
    lambdaMaxSharpness: 'λmax (锐度): Hessian 的最大特征值，追踪损失景观中最尖锐的方向。',
    stabilityBoundary: '稳定性边界 (2/η): 如果 λmax 超过此值，普通 SGD 的训练可能变得不稳定。',
    traceDesc: 'Trace: 特征值之和，表示当前点周围的总曲率。',
    sharpness: '锐度 (λmax)',
    stability: '稳定性 (2/η)',
    totalCurvature: '总曲率 (Trace)',
    spectralAnalysis: '谱分析',
    spectralDensityDesc: '谱密度: Hessian 特征值的 KDE；当前 epoch 被高亮显示。',
    screePlotDesc: '碎石图: 顶部特征值（代数顺序）以区分主体谱和异常值。',
    spectralDensity: '谱密度 (KDE)',
    epoch: 'Epoch',
    history: '历史',
    noDensityData: '无密度数据可用',
    topKSpectrum: 'Top-k 谱（已排序）',
    rank: '排名',
    eigenvalue: '特征值',
    noSpectrumData: '无谱数据可用',
    density: '密度',
    currentPosition: '当前位置',
    trainingTrajectory: '训练轨迹',
    baselineLoss: '基线 Loss',
    highLoss: '高 Loss',
    lowLoss: '低 Loss',
    scale: '比例',
    log: '对数',
    linear: '线性',
    tipHover: '提示: 将鼠标悬停在图表上以查看精确值',
    tip3D: '提示: 拖动旋转，滚动缩放，悬停查看值',
    hoverForHint: '悬停查看提示',
    lossLandscape2D: 'Loss Landscape (2D)',
    lossLandscape3D: 'Loss Landscape (3D)',
    lowerIsBetterArrow: '↓ 越低越好 ↓',
    noData: '无数据',
    tabRun: '运行',
    tabHistory: '历史',
    tabUpload: '上传',
    loss: '损失',
    trainLoss: '训练损失',
    validationLoss: '验证损失',
    alpha: 'α',
    beta: 'β',
    log10Loss: 'log₁₀(损失)',
    sectionExperiment: '实验',
    sectionDataset: '数据集',
    sectionModel: '模型',
    sectionTraining: '训练',
    sectionTrainingStatistics: '训练统计',
    sectionLossFunction: '损失函数',
    sectionLandscapeGeneration: 'Landscape 生成',
    sectionSystemInformation: '系统信息',
    alphaCurve: 'Alpha 曲线',
    alphaFloor: 'Alpha 底限',
    metrics: '指标',
    trainingProgression: '训练进度',
    noHessianMetrics: '此运行没有 Hessian 指标。',
    noHessianDataAvailable: '无 Hessian 数据可用',
    view1D: '1D',
    view1DDesc: '1D Loss Landscape 可视化',
    viewHessian: 'Hessian',
    viewHessianSnapshot: 'Hessian 快照（无轨迹）',
    viewHessianMetrics: '训练过程中的 Hessian 指标 (λmax, Trace, Spectrum)',
    viewMetadata: '元数据',
    viewMetadataDesc: '查看完整的运行元数据',
    slices: '切片',
    volumeRenderingDesc: '体积渲染：所有切片混合在一起。越亮 = 损失越高。旋转查看 3D 结构。',
    landscapeLoss: 'Landscape 损失',
    valueLabelTrain: '训练',
    valueLabelVal: '验证',
    valueLabelLand: 'Landscape',
    no1DLossData: '此运行没有 1D loss landscape 数据。',
    items: '项',
    modalOK: '确定',
    modalCancel: '取消',
    noMetadataAvailable: '无元数据可用',
    noMetadataInfo: '此运行没有元数据信息。',
    no1DDataAvailable: '无 1D 数据可用',
    notAvailable: '不可用',
    sliceIndex: '切片索引',
    alphaSlice: 'α-切片',
    betaSlice: 'β-切片',
    gammaSlice: 'γ-切片',
    alphaIdx: 'α 索引',
    betaIdx: 'β 索引',
    gammaIdx: 'γ 索引',
    axis: '轴',
    threshold: '阈值',
    dataMinMax: '数据最小/最大值:',
  },
} as const satisfies Record<Locale, Record<string, string>>;

type DictKey = keyof typeof DICT.en;
type Dict = Record<DictKey, string>;

const I18nCtx = createContext<{
  locale: Locale;
  setLocale: (l: Locale) => void;
  t: Dict;
} | null>(null);

export function I18nProvider({ children }: { children: React.ReactNode }) {
  const [locale, setLocale] = useState<Locale>('zh');

  useEffect(() => {
    const saved = typeof window !== 'undefined' ? window.localStorage.getItem('locale') : null;
    if (saved === 'en' || saved === 'zh') setLocale(saved);
  }, []);

  useEffect(() => {
    if (typeof window !== 'undefined') window.localStorage.setItem('locale', locale);
  }, [locale]);

  const value = useMemo(() => ({ locale, setLocale, t: DICT[locale] }), [locale]);
  return <I18nCtx.Provider value={value}>{children}</I18nCtx.Provider>;
}

export function useI18n() {
  const ctx = useContext(I18nCtx);
  if (!ctx) throw new Error('useI18n must be used within I18nProvider');
  return ctx;
}


